...
1. Message: Initial email + Follow-up steps in st.tabs() — same row, each editable separately.
   ➕ Add follow-up button adds a new tab (up to 3 follow-ups).
2. Recipients: Filter by tags, status/stage, hand-pick with version-keyed checkboxes.
3. Mailboxes & Pacing: Round-robin fleet, review card, overflow, Schedule batch CTA.

Schedule batch queues BOTH initial emails AND all follow-up emails in one click.
"""
...
def render_bulk_tab():
    """Render the 3-section Bulk Send screen with follow-up sequence support."""
    ...
    # =========================================================================
    # STEP 1 · MESSAGE — Initial email + Follow-up tabs in one row
    # =========================================================================
    # Header row: mode toggles  +  ➕ Add follow-up  +  🗑 Clear follow-ups
    hdr_c1, hdr_c2, hdr_gap, hdr_add, hdr_clear = st.columns(
        [1.2, 1.2, 2.5, 1.5, 1.1], vertical_alignment="center"
    )
    ...
    with hdr_c1:
        ...
        if st.button(
            "📄 Load a template", key="bulk_mode_tpl",
    ...
    with hdr_c2:
        if st.button(
            "✍️ Write your own", key="bulk_mode_custom",
    ...
    with hdr_add:
        ...
        if st.button(
            ...
            help="Adds a follow-up tab — edit subject, body and send delay separately"
        ):
            delay = (fu_count + 1) * 3          # default: +3 days, +6 days, +9 days
            ...
            st.session_state["bulk_followup_steps"].append({
                ...
                "body":       (
                    "Hi [Name],\n\n"
                    "Just following up on my previous email — wanted to make sure it didn't get lost.\n\n"
                    "Happy to keep this brief. Would love to connect if the timing works.\n\n"
                    "Best,"
                ),
            })
            st.rerun()
    ...
    with hdr_clear:
        if fu_count > 0:
            if st.button("◻️ Clear all", key="bulk_clear_fu", use_container_width=True,
    ...
    with tabs[0]:
        ...
        if st.session_state["bulk_mode"] == "template":
            if not all_templates:
                st.warning("No saved templates found. Create one in **Templates** or switch to 'Write your own'.")
                selected_subject = ""
                current_body     = ""
            else:
                tpl_choices = {
        ...
        st.markdown(
            ...
            "Edit the loaded template or compose from scratch — same editor as Compose. Spam-checked before send."
            "</div>",
            unsafe_allow_html=True
        )

    # ── Follow-up tabs (one per step) ────────────────────────────────────
    for i, step in enumerate(st.session_state["bulk_followup_steps"]):
        with tabs[i + 1]:
            # Delay slider + Remove button in one row
            fu_top_c1, fu_top_c2 = st.columns([4, 1], vertical_alignment="center")
            with fu_top_c1:
            ...
            with fu_top_c2:
                if st.button("🗑️ Remove", key=f"bulk_fu_{i}_rm",
                             use_container_width=True,
                             help=f"Remove follow-up #{i + 1}"):
                    st.session_state["bulk_followup_steps"].pop(i)
                    st.rerun()

            # Subject & body — editable independently from the initial email
            step["subject"] = st.text_input(
                "Follow-up subject",
                value=step["subject"],
                key=f"bulk_fu_{i}_subj",
                help="Supports [Name], [Company] and Spintax"
            )
            step["body"] = st.text_area(
                ...
                help="Supports [Name], [Company] and other CRM tokens",
                label_visibility="visible"
            )

            # Quick token chips
            chip_c = st.columns([1, 1.2, 1.2, 4])
            with chip_c[0]:
                if st.button("👤 [Name]",    key=f"bulk_fu_{i}_chip_name",    use_container_width=True):
                    step["body"] = st.session_state.get(f"bulk_fu_{i}_body", step["body"]) + " [Name]"
                    st.session_state[f"bulk_fu_{i}_body"] = step["body"]
                    st.rerun()
            with chip_c[1]:
                if st.button("🏢 [Company]", key=f"bulk_fu_{i}_chip_comp",    use_container_width=True):
                    step["body"] = st.session_state.get(f"bulk_fu_{i}_body", step["body"]) + " [Company]"
                    st.session_state[f"bulk_fu_{i}_body"] = step["body"]
                    st.rerun()
            with chip_c[2]:
                if st.button("🖋️ Signature", key=f"bulk_fu_{i}_chip_sig",     use_container_width=True):
                    ...
                    step["body"] = st.session_state.get(f"bulk_fu_{i}_body", step["body"]) + f"\n\n{sig}"
                    st.session_state[f"bulk_fu_{i}_body"] = step["body"]
                    st.rerun()

            st.caption(
                f"📅 Follow-up #{i + 1} will dispatch **{step['delay_days']} day(s)** after the initial batch — "
                f"same recipients, independent message."
            )
    ...
    with col_pacing:
        ...
        if st.button("🚀 Schedule batch", type="primary", use_container_width=True, key="bulk_schedule_cta"):
            ...
            with st.spinner("Scheduling batch outreach..."):
                queued_count  = 0
                fu_queued     = 0
                base_start_dt = datetime.combine(chosen_date, chosen_time)
                ...
                for i, lead in enumerate(selected_leads):
                    ...
                    if i < total_fleet_cap:
                        target_dt = base_start_dt + timedelta(seconds=(i * step_seconds))
                    else:
                        overflow_offset = i - total_fleet_cap
                        target_dt = (base_start_dt + timedelta(days=1)) + timedelta(seconds=(overflow_offset * step_seconds))
                ...
                # ── Follow-up emails ──────────────────────────────────────
                for step in st.session_state["bulk_followup_steps"]:
                    ...
                    for i, lead in enumerate(selected_leads):
                        lead_tz   = lead.get("country_or_timezone") or "LOCAL"
                        # Base on initial send time + delay_days + proportional spread offset
                        fu_target = (
                            base_start_dt
            ...
            # Clear follow-up steps after successful schedule
            st.session_state["bulk_followup_steps"] = []
